...
- **Canvas 模式**：与经典工作台同级的工作台模式，由 CanvasShell 承载项目级导航，由 CanvasWorkspace 承载画布优先界面、底部生图 dock、右侧画布助手 / Canvas Agent、本地 Canvas project、默认项目创建/恢复、无限画布视口、文本 / 图片 / 生成 / 配置 / 批量 / 结果节点、节点动作条、连接到空白创建下游节点、生成节点输入摘要、多结果 result 节点组织、轻量连线、当前参考图 / 历史图 / 图库图 / 本地图片加入 Canvas 的素材桥、文本一键生成、手动单节点生成、Canvas project JSON 导入导出、助手会话本地持久化，以及有上限的顺序 workflow run。
- **画布助手 / Canvas Agent**：CanvasWorkspace 右侧常驻的会话调度面板；配置 Canvas Agent Provider 时通过原生 tool calling 调用受控 Canvas 工具，展示运行 timeline、pending change 和节点定位高亮；未配置或 Provider 不支持时保留本地规则解析兜底。助手消息按 project 写入独立本地会话存储，支持最近页加载、加载更早和清空。
...
- **Provider 管理流**：在全局设置窗内维护图片 / 提示词 / Canvas Agent Provider 的默认选择、列表维护与编辑弹窗。
...
- **Canvas project shell / basic nodes / node action toolbar / connection authoring / generation context / result composition / reference bridge / generate node / history gallery integration / project import-export / advanced workflow nodes / canvas assistant / canvas agent**
  - `src/components/canvas/`
  - `src/store/canvas-store.ts`
  - `src/services/canvas-projects.ts`
  - `src/services/canvas-assistant-sessions.ts`
  - `src/services/canvas-workflow.ts`
  - `src/services/canvas-assistant.ts`
  - `src/services/canvas-agent-service.ts`
  - `src/services/canvas-agent-runner.ts`
  - `src/services/canvas-agent-tools.ts`
  - `src/services/canvas-agent-context.ts`
  - `src/lib/platform.ts`
  - `src-tauri/src/lib.rs`
  - `src/shared/generation-origin.ts`
  - `src/services/image-service.ts`
  - `src/services/app-database.ts`
  - `src/services/provider-settings.ts`
  ...
  - 负责 Canvas 模式入口、画布优先 workspace、底部生图 dock、右侧画布助手 / Canvas Agent、默认项目创建/恢复、本地 `pixai-canvas-projects` 状态、助手消息本地 SQLite 会话存储、`{ x, y, k }` 视口持久化、文本/图片/生成/配置/批量/结果节点、节点动作条、轻量连线、连接到空白创建合法下游节点、助手指令解析与执行回执、Canvas Agent Provider 选择、OpenAI-compatible 原生 tool calling、Canvas summary/inspect、受控工具 registry、最多 8 次工具调用的 runner、运行 timeline、pending prompt change、节点定位/短时高亮、生成节点输入摘要、多图/批量生成结果追加 result 节点、文本节点一键创建下游生成节点、图片/结果节点预览和 mask、生成节点运行/重试、从当前参考图、历史成功图、图库图或本地图片创建带来源绑定的 Canvas 图片节点、手动单节点生成到 history/Canvas result node 的桥接、Canvas-origin history/gallery 标识与来源搜索、Canvas project JSON 文本导入导出，以及 8 次请求以内的顺序 workflow run。
- **流式预览链路**
  - `src/lib/platform.ts`
  - `src/adapters/openai-compatible.ts`
  - `src/services/image-service.ts`
...
- CanvasWorkspace 的画布助手是 Canvas 专属右侧面板，不复用 Workspace 参数栏；配置 Canvas Agent Provider 后优先走 `CanvasAgentRunner` + 原生 tool calling，未配置或不可用时保留本地规则解析兜底。
...
- Provider 维护不再和工作区参数混排，而是作为 `Services` 分区内的独立管理流存在；Canvas Agent 使用独立默认 Provider 选择，不复用 Prompt Provider 语义。
...
- Canvas project 导入导出第一版只处理 JSON 文本快照；导入永远 clone 为新 project，刷新 project id，并绑定当前 active conversation；导出默认不包含助手聊天历史。
...
- 画布助手只操作 active Canvas project；助手消息写入独立本地会话存储，不写入 Canvas project JSON，不作为跨 project 调度入口，也不做云同步。Canvas Agent 只暴露 V1 受控工具，不提供删除节点/连接、批量破坏性操作或复杂 workflow 编排；V1 只支持原生 tool calling，不做 JSON plan fallback，confirm 工具不由模型自动执行。
...
- Canvas project 绑定创建或导入时的当前会话，不创建隐藏会话；当前支持文本/图片/生成/配置/批量/结果节点、右侧画布助手 / Canvas Agent、节点动作条、连接到空白创建合法下游节点、生成节点输入摘要、多图/批量结果追加 result 节点、轻量连线、当前参考图/历史图/图库图/本地图片到 Canvas image/result node 的桥接、文本一键生成、手动单节点生成、图库来源标识、JSON 导入导出和 8 次请求以内的顺序 workflow run；不支持端口体系、复杂 DAG 调度、并发队列、后台批量调度、复杂 workflow agent、复杂结果分组折叠、从图库跳回 Canvas 节点、节点级取消 UI、视频/音频节点或助手动作、带图片资源包的项目包、云同步或批量导入导出。
